...
To edit the `api.yml` definition file, you can use a tool such as [-Editor](). Start a local instance of the swagger-editor using docker by running: `docker-compose -f src/main/docker/swagger-editor.yml up -d`. The editor will then be reachable at [http://localhost:7742](http://localhost:7742).
...
To build the final jar and optimize the formBook application for production, run:
